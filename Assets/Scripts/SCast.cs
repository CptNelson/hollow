...
{ (0,0), (N,0), (N,N) } on a Cartesian plane.  The cell _tilemap identifies the center of 1x1
...
- Access to the map is provided by an "ICell_tilemap" implementation, which has two methods:
  ...
  and two properties:
    int xDim, yDim - _tilemap dimensions in the X and Y directions
  Coordinates are range-checked, so the methods will only be called with valid values.

- Uses a trivial "Vector3Int" class, which just holds a pair of integers.
...
public static class SCast
{
    ...
    private static void CastLight(Tilemap _tilemap, Vector3Int _tilemapPosn, float viewRadius,
            int startColumn, float leftViewSlope, float rightViewSlope, OctantTransform txfrm)
    {
        ...
        int xDim = GameMaster.width;
        int yDim = GameMaster.height;
        ...
        for (int currentCol = startColumn; currentCol <= viewCeiling; currentCol++)
        {
            ...
            {
                // Translate local coordinates to _tilemap coordinates.  For the various octants
                // we need to invert one or both values, or swap X for Y.
                int _tilemapX = _tilemapPosn.x + xc * txfrm.xx + yc * txfrm.xy;
                int _tilemapY = _tilemapPosn.y + xc * txfrm.yx + yc * txfrm.yy;

                // Range-check the values.  This lets us avoid the slope division for blocks
                // that are outside the _tilemap.
                //
                // Note that, while we will stop at a solid column of blocks, we do always
                // start at the top of the column, which may be outside the _tilemap if we're (say)
                // checking the first octant while positioned at the north edge of the map.
                if (_tilemapX < 0 || _tilemapX >= xDim || _tilemapY < 0 || _tilemapY >= yDim)
                {
                    continue;
                }
                ...
                else
                {
                    if (curBlocked)
                    {
                        ...
                        {
                            CastLight(_tilemap, _tilemapPosn, viewRadius, currentCol + 1,
